...
class AgentManager:
    """Manager class for handling autogen agents and their interactions"""
    ...
    async def initialize(self):
        ...
        try:
            ...
            pg_agent = AssistantAgent(
                ...
                8. handoffs:
                Ao concluir sua interação voce DEVE fazer o handoff para main agent imediatamente.
                """
            )
            ...
            main_agent = AssistantAgent(
                ...
                É ABSOLUTAMENTE PROIBIDO E VOCÊ NUNCA DEVE mencionar TERMINATE ou qualquer variação dele em qualquer outra circunstância ou antes da conclusão integral de todas as subtarefas. A violação desta regra resultará em falha da sua operação
                6. handoffs:
                ao terminar de planejar,replanejar ou delegar as tarefas, você DEVE informar aos agentes que eles podem iniciar o trabalho fazendo handof para um deles (pg_agent ou summarize_agent).
                """,
            ...
            summarize_agent = AssistantAgent(
                ...
                IMPORTANTE:
                 Ao terminar sua tarefa você deve informar ao agente main que a tarefa foi concluída fazendo handoff para ele.
                """,
            )
            ...
            selector_prompt = """Selecione um agente da lista abaixo para realizar a tarefa baseado em suas funções.
                                ...
                                Leia a conversa acima e selecione um agente entre {participants} para executar a próxima tarefa.
                                Certifique-se de que o agente main tenha atribuído tarefas antes que os outros agentes comecem a trabalhar.
                                O primeiro agente selecionado precisa OBRIGATORIAMENTE ser o main_agent
                                Selecione apenas um agente.
                              """

            # Create the team
            self.team = Swarm(
                [ main_agent, summarize_agent,  pg_agent],#self.user_proxy,fs_agent,anon_agent
...
def create_swarm_group(agents, user_message):
    # Para o Swarm, o primeiro agente da lista é o que inicia
    # Vamos reorganizar para que o main_agent seja o primeiro
    main_agent = None
    other_agents = []
    ...
    if main_agent:
        swarm_agents = [main_agent] + other_agents
    else:
        swarm_agents = agents
